...
class Splitter(object):
	def __init__(self, X_train, reducer_name, network_architecture,
							 clustering):

		self.X_train = standard(X_train)
...
#		self.models = []
	...
	def __del__(self):
		...
		del self.n_ftrs
		del self.t_ind
		del self.reducer_name
		del self.network_architecture
		del self.clustering
		del self.step
		del self.X
		del self.z_mu
	...
	def _process(self, X_in,clustering,
		...
		if self.reducer_name=='AE':
			model = AutoEncoder(self.network_architecture)
		elif self.reducer_name=='VAE':
			model = VariationalAutoEncoder(self.network_architecture)
		else:
			assert False, 'Unknown reducer!'

		mean_points = np.zeros((2,2))
		mean_components = np.zeros((2, self.n_ftrs))
		dense_points = np.zeros((2,2))
		...
		if X.shape[0]==1:
			...
			for j in [0]:
				...
				mean_vector[j,:] = X
			return X,z_mu,np.array([0]),z_mu,mean_components,z_mu,dense_components,mean_vector,covariance
		...
		for j in [0,1]:
			...
			if pts.shape[0]!=0:
				...
				mean_components[j,:] = model.decoder((mean_points[j,:]).reshape(1,2))
				dense_points[j,:] = dense_point(pts)                             
				dense_components[j,:] = model.decoder((dense_points[j,:]).reshape(1,2))
		...
		return X,z_mu,label_out,mean_points,mean_components,dense_points,dense_components,mean_vector,covariance #,model
	...
	def split(self, n_s, 
		...
		if self.step==0:
			...
			self.mean_points = [mean_points.reshape(1,2,2)]
			self.mean_components = [mean_components.reshape(1,2,self.n_ftrs)]
			self.dense_points = [dense_points.reshape(1,2,2)]
...
class Custom_Splitter(object):
	def __init__(self, X_train, reducer, clustering):
