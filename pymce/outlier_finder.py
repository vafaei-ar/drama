...
def outliers(X,splitter,metrics,represent='dense'):
	...
	if represent=='dense':
		components = splitter.dense_components[-1].reshape(-1,n_ftrs)
	elif represent=='mean':
		components = splitter.mean_components[-1].reshape(-1,n_ftrs)
	else:
		assert False, 'Representative selection method is not recognized!'
